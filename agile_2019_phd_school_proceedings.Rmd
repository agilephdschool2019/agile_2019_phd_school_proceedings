...
for (i in 1:nrow(articles)) {
  ...
  path <- repo$path
  if(dir.exists(path)) {
    cat("Skipping existing directory ", path, "\n")
    next()
  }
...
```{r ceur_template}
library("rmarkdown")
remotes::install_local("template/ceurticles", force = TRUE)
```

1. identify "main file"
2. create PDF for each article using the template in the R package `template/ceurticles`
  1. copy template files to the paper directory
  2. render document

```{r rmd_file}
# rename wrong input files
if(file.exists("article_repos/Bruno_Montibeller/notes_agile.Rmd"))
  file.rename(from = "article_repos/Bruno_Montibeller/notes_agile.Rmd", "article_repos/Bruno_Montibeller/notes_agile.Rmd.dontuse")
...
```{r articles}
articles %>%
  dplyr::mutate(path = sapply(articles$path, relative_to, dir = here(), USE.NAMES = FALSE))
```
...
3. Upload the submission ZIP to the GitHub release
